...
@dataclass(frozen=True)
class SchemeWeight:
    """MODULE_5.md §8.1 — one scheme's position in one issuer, across the market.

    The flow engine's unit of observation. `weight` is a percentage of the
    holding scheme's NAV.
    """
...
@dataclass(frozen=True)
class Constituent:
    """MODULE_0.md `index_constituent`.

    `BUILD_ORDER.md` R4 promotes this to a V1 dependency: betweenness 0.164,
    bridging M0, M2, M3 and M5. Without it `sector_tilt` renders absolute
    exposure with no comparison.
    """
    ...
    free_float_mcap_inr: Decimal | None


@dataclass(frozen=True)
class DirectHolding:
    """MODULE_1.md `direct_holding`.

    In scope only because combined exposure is the point: holding a bank
    directly *and* via five funds is one number, not two (`PLAN.md` §3.1).
    """
