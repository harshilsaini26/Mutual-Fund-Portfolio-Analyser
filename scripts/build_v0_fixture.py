...
class Redemption(NamedTuple):
    """What the report needs to explain one redemption's exit load."""
...
PREAMBLE = """> V0.1 transactions, priced on the REAL NAV series in nav_series.yaml.
...
def add_hdfc(rows: Rows, h: dict[date, Decimal]) -> Redemption:
    """Six monthly SIP instalments, a bounced seventh, and a straddling sale."""
    ...
    return Redemption(r_day, r_nav, r_gross, r_load, r_inside, r_units)


def add_switch(
    rows: Rows, h: dict[date, Decimal], i: dict[date, Decimal]
) -> None:
    """HDFC -> ICICI, on a day BOTH schemes priced.

    Taxed on the way out and the holding clock restarts on the way in, which
    is why it is two rows sharing a switch_group_id rather than one.
    """
...
def add_kotak(
    rows: Rows, k: dict[date, Decimal]
) -> tuple[list[dict[str, object]], Redemption]:
    """A 0.5% exit load and a NAV two orders of magnitude smaller.

    NAV here is ~Rs 20-40 against HDFC's ~Rs 2,000, so Rs 50,000 buys hundreds
    of units rather than a handful. Unit quantisation to 6dp bites differently
    at that scale, and the exit load is HALF the other schemes'.
    """
    ...
    return [k1, k2], Redemption(kr_day, kr_nav, kr_gross, kr_load, kr_inside, kr_units)
...
def report(
    ...
    hdfc: Redemption,
    kotak: Redemption,
    kotak_lots: list[dict[str, object]],
) -> None:
    print(f"Generated transactions.csv — {n_rows} rows\n")
    print("HDFC redemption")
    print(f"  {hdfc.day} nav={hdfc.nav} gross={hdfc.gross}")
    print(f"  exit load {hdfc.load} at 1.0% on {hdfc.inside} of {hdfc.units} units")
    print("Kotak redemption")
    print(f"  {kotak.day} nav={kotak.nav} gross={kotak.gross}")
    print(f"  exit load {kotak.load} at 0.5% on {kotak.inside} of {kotak.units} units")
    print("\nKotak lots")
    for n, kb in enumerate(kotak_lots, 1):
        days = (kotak.day - kb["date"]).days  # type: ignore[operator]
        print(
...
def main() -> None:
    ...
    hdfc = add_hdfc(rows, h)
    add_switch(rows, h, i)
    kotak_lots, kotak = add_kotak(rows, k)

    body = PREAMBLE + ",".join(Rows.COLUMNS) + "\n" + "\n".join(rows.lines) + "\n"
    (FIXTURES / "transactions.csv").write_text(body, encoding="utf-8")

    report(len(rows.lines), hdfc, kotak, kotak_lots)
